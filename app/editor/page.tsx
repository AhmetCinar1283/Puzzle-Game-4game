...
function EditorInner() {
  ...
  // Submit dialog (community level request)
  ...
  // Load parts list once when admin/moderator
  useEffect(() => {
  ...
  // Reload Firestore level list when part or toggle changes
  useEffect(() => {
    if (!isModerator || !showFirestoreLevels) return;
    (async () => {
      const { getPartLevels } = await import('@/app/src/lib/firebase/admin');
      const levels = await getPartLevels(selectedPartId);
      setFirestoreLevels(levels);
    })();
  ...
  // ── Submit level request (community) ────────────────────────────────────
  ...
  // ── Load a Firestore level into the editor (admin) ───────────────────────
  ...
  // ── Publish / update level to Firestore (admin) ───────────────────────────
  const doPublish = useCallback(async () => {
    ...
    // Refresh Firestore level list if visible
  ...
  // ── Cell size (viewport-aware) ────────────────────────────────────────
  const [cellSize, setCellSize] = useState(48);
  useEffect(() => {
    function compute() {
      const vw = window.innerWidth;
      const vh = window.innerHeight;
      // Mobile: tool palette=110px, tab bar~40px, top bar~45px, padding~20px
      // Desktop: left panel=170px, tool palette=110px, top bar~45px, right panel~220px
      const mob = vw < 900;
      const availW = mob ? vw - 110 - 20 : vw - 170 - 110 - 220 - 20;
      const availH = vh - (mob ? 130 : 80);
      const cs = Math.max(24, Math.min(58,
        Math.floor(availW / width),
        Math.floor(availH / height),
      ));
      setCellSize(cs);
    }
    compute();
    ...
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: '#030712', color: '#e2e8f0', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: isMobile ? '100%' : 170, flexShrink: 0, borderRight: isMobile ? 'none' : '1px solid rgba(30,58,95,0.4)', display: isMobile ? (activeTab === 'levels' ? 'flex' : 'none') : 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {/* Admin: Firestore levels toggle */}
          {isModerator && (
            <div style={{ borderTop: '1px solid rgba(30,58,95,0.3)', padding: '6px 8px' }}>
              <button
                onClick={() => setShowFirestoreLevels((v) => !v)}
                style={{ width: '100%', padding: '5px 8px', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', background: showFirestoreLevels ? 'rgba(251,191,36,0.1)' : 'rgba(251,191,36,0.03)', border: `1px solid ${showFirestoreLevels ? 'rgba(251,191,36,0.5)' : 'rgba(251,191,36,0.2)'}`, color: '#fbbf24', borderRadius: 6, cursor: 'pointer' }}
              >
              ...
              {showFirestoreLevels && (
                <div style={{ marginTop: 4 }}>
                  {firestoreLevels.length === 0 ? (
                    <span style={{ fontSize: 10, color: '#334155', padding: '4px 2px', display: 'block' }}>No levels in Part {selectedPartId}</span>
                  ) : firestoreLevels.map((fl, idx) => (
                    <button
                      key={fl.firestoreId}
                      onClick={() => loadFirestoreLevel(fl)}
                      style={{ width: '100%', marginBottom: 3, padding: '6px 8px', textAlign: 'left', background: firestoreEditId === fl.firestoreId ? 'rgba(251,191,36,0.1)' : 'rgba(255,255,255,0.02)', border: `1px solid ${firestoreEditId === fl.firestoreId ? 'rgba(251,191,36,0.4)' : 'rgba(255,255,255,0.06)'}`, color: firestoreEditId === fl.firestoreId ? '#fbbf24' : '#64748b', borderRadius: 5, cursor: 'pointer', fontSize: 11 }}
                    >
        ...
        <div style={{ width: isMobile ? '100%' : 270, flexShrink: 0, borderLeft: isMobile ? 'none' : '1px solid rgba(30,58,95,0.4)', overflowY: 'auto', padding: '12px 14px', display: isMobile ? (activeTab === 'settings' ? 'block' : 'none') : 'block' }}>
          ...
          {/* Admin-only: Firestore publish */}
          {isModerator && (
            <Sec title="Firestore Publish">
              <div style={{ marginBottom: 8 }}>
                <Lbl>Part</Lbl>
                <select
                ...
                >
                  {parts.length === 0 ? (
                    <option value="1">Part 1 (default)</option>
                  ) : (
                    parts.map((p) => (
              ...
              {firestoreEditId && (
                <div style={{ fontSize: 9, color: '#fbbf24', marginBottom: 8, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.2)', borderRadius: 5, padding: '4px 8px' }}>
                  Editing FS: {firestoreEditId.slice(0, 10)}…
                  <button
                    onClick={() => setFirestoreEditId(null)}
                    style={{ marginLeft: 6, fontSize: 9, background: 'none', border: 'none', color: '#475569', cursor: 'pointer' }}
                  >✕ new</button>
                </div>
              )}
              <button
              ...
              >
                {publishStatus || (firestoreEditId ? '↑ Update Firestore' : '⚡ Publish to Firestore')}
              </button>
            </Sec>
          )}
