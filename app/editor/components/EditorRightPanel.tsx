...
import { EDGE_OPTIONS, EDGE_LABEL, EDGE_COLOR, DIFFICULTY_LABELS, DIFFICULTY_COLORS, type ObjConfig, type BoxConfig, type ToolType } from '../editorConfig';
    ...
    <div style={{
      ...
      <Sec title="Actions">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          <button onClick={onTest} style={{ padding: '9px', fontSize: 12, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', background: 'rgba(0,255,136,0.06)', border: '1px solid rgba(0,255,136,0.35)', color: '#00ff88', borderRadius: 8, cursor: 'pointer' }}>▶ Test Level</button>
        ...
        <Sec title="Firestore Publish">
          ...
          {firestoreEditId && (
            <div style={{ fontSize: 9, color: '#fbbf24', marginBottom: 8, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.2)', borderRadius: 5, padding: '4px 8px' }}>
              ...
              <button onClick={() => setFirestoreEditId(null)} style={{ marginLeft: 6, fontSize: 9, background: 'none', border: 'none', color: '#475569', cursor: 'pointer' }}>✕ new</button>
            </div>
          )}
          <button
            onClick={onPublish}
