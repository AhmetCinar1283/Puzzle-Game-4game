...
import { DELTA, cellTypeToConveyorDir } from '../positionUtils';
...
function collectPushChain(
  ...
  // ── Conveyor push check ──────────────────────────────────────────────────
  // If the box is on a conveyor belt, it cannot be pushed against the conveyor's direction.
  const currentCellType = tick.grid[box.position.row]?.[box.position.col]?.type;
  if (currentCellType) {
    const convDir = cellTypeToConveyorDir(currentCellType);
    if (convDir) {
      const opposite: Record<Direction, Direction> = { up: 'down', down: 'up', left: 'right', right: 'left' };
      if (direction === opposite[convDir]) {
        return null; // Block the push!
      }
    }
  }
